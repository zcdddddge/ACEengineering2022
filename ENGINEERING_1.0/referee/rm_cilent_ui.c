...
{
    static u8 msg[ALL_GRAPH_LEN];
    UI_Packhead framehead;
    framehead.SOF = UI_SOF;
    framehead.Data_Length = UI_DATA_OPERATE_LEN + GRAPH_DATA_LEN;
    framehead.Seq = UI_Seq;
    framehead.CRC8 = Get_CRC8_Check_Sum((u8 *)&framehead, 4, 0xff);
    framehead.CMD_ID = 0x0301;

    UI_Data_Operate data_operate;
    data_operate.Data_ID = UI_Data_ID_Draw1;
    data_operate.Sender_ID = referee_robot_id();
    if (referee_robot_id() == UI_Data_RobotID_BHero) //if robot is blue hero
	...
	uart4_dma_send(msg, ALL_STRING_LEN);
    //DMA_printf(DMA_REFEREE_STREAM, USART_REFEREE, msg, ALL_GRAPH_LEN);
    UI_Seq++;
    vTaskDelay(10);
}
...
void My_Char_Refresh(String_Data string)
{
	...
	uart4_dma_send(msg, ALL_STRING_LEN);
    //DMA_printf(DMA_REFEREE_STREAM, USART_REFEREE, msg, ALL_STRING_LEN);
    UI_Seq++;
    vTaskDelay(10);
}
