...
void USART6_RC_INIT_PC7_RX(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART6,ENABLE);    //时钟使能
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL; 
	GPIO_Init(GPIOC,&GPIO_InitStructure);        //复用io口配置
	...
	DMA_ITConfig(DMA2_Stream1,DMA_IT_TC,ENABLE);  //dma传输完成产生中断
